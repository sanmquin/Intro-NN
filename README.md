...
## Task 6: Mechanistic Interpretability on a Tiny Network (The Fourth Notebook)
We created a new notebook `4.small_network_analysis_tutorial.ipynb` (generated by `create_small_network_notebook.py`) to dissect and study an extremely small neural network: **1 hidden layer with only 6 hidden neurons** (total of 92 parameters) on the same Gaussian parameter estimation task.

### Key Analytical Findings:

#### A) How Training Changed (Low-Capacity Constraints)
- **Representational Bottleneck**: Standard architectures have large neuron budgets (e.g. 64 or 128 units) to approximate functions. With only 6 neurons, the network has only 6 piecewise linear basis functions (ReLUs) to partition the 12-dimensional sample space.
- **Plateau and Loss Curves**: Training and validation losses converge much slower and plateau at a significantly higher minimum error ($\approx 0.15 - 0.25$) compared to larger models ($\approx 0.05$).
- **Linear vs. Quadratic Asymmetry**:
  - **Mean prediction ($\mu$)** is linear and remains exceptionally strong ($R^2 \approx 0.94 - 0.95$), nearly matching the optimal classical sample mean baseline ($R^2 \approx 0.9505$).
  - **Variance prediction ($\sigma^2$)** is quadratic and drops significantly ($R^2 \approx 0.38 - 0.45$). This tiny network struggles to exceed or even match the classical sample variance baseline ($R^2 \approx 0.4466$).

#### B) Connectivity (Weights) with Inputs & Outputs
Dissecting the input-to-hidden weight matrix $W_1$ (shape `6x12`) and hidden-to-output weight matrix $W_2$ (shape `2x6`) reveals two distinct physical connectivity patterns:
1. **Mean Detectors**: Units where input weights have the same sign and almost uniform magnitudes (e.g., all $\approx +0.28$ or $-0.28$). These act as sequence integrators. In $W_2$, their outputs connect directly to the "Mean Output" with high-magnitude projection weights.
2. **Variance Detectors (Contrast Filters)**: Units with high-contrast alternating positive and negative input weights. These act as "difference filters" to capture the dispersion/spread of sample sequences. In $W_2$, they project with heavy positive weights to the "Variance Output".

#### C) Influence on Inference (Mechanistic Specialization)
We evaluated the activations of all 6 individual neurons across the test set and computed their Pearson correlation with the true target parameters:
- **Directional Mean Detectors**: Exhibit high Pearson correlation with the mean ($r \approx \pm 0.95$). Because of the ReLU rectification, one detector fires for positive means (a monotonic line for $\mu > 0$), and another fires for negative means ($\mu < 0$).
- **Non-Directional Variance Detectors**: Exhibit high correlation with the true variance and low correlation with the mean. Plotted against mean, they exhibit symmetric **"U-shape"** or **"V-shape"** curves, showing that they fire when sample inputs deviate from zero in either direction (spread detection).
- **Dead Neurons**: In some initializations, one or more neurons suffer from dead ReLU activation, firing a constant $0.0$. In a tiny 6-neuron model, a single dead neuron represents a severe $16.7\%$ reduction in network capacity.

### Charts & Outputs Generated:
- `small_loss_comparison.png`: Loss trajectory showing slow convergence and higher plateau.
- `small_prediction_scatters.png`: True vs. predicted scatters for both Mean and Variance targets.
- `small_connectivity_heatmaps.png`: Connection weights of $W_1$ and $W_2$, highlighting mean detection and contrast filter configurations.
- `small_neuron_influence_scatters.png`: Test set activations of all 6 hidden neurons against True Mean and True Variance, visually proving their functional specialization.
...
# Generate Small Network Dissection Notebook
python3 create_small_network_notebook.py
