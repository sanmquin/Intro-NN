...
export default function ArchitectureDiagram({
  ...
  const blocks = [
    ...
    ...Array.from({ length: numLayers }).map((_, lIdx) => ({
      id: `layer${lIdx + 1}`,
      layerIndex: lIdx,
      name: `Transformer Layer ${lIdx + 1}`,
      desc: `Pre-LN Multi-Head Attention (${numHeads} Head${numHeads > 1 ? 's' : ''}, ${32 / numHeads}D each) & GELU FFN.`,
    ...
    <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-6 backdrop-blur-sm">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 justify-between bg-zinc-950 p-6 rounded-xl border border-zinc-800">
        ...
        <div className="flex flex-col items-center gap-1 min-w-[100px] text-center">
          ...
          <div className="flex gap-1.5 mt-1 bg-zinc-900 p-2 rounded-lg border border-zinc-800">
            {trace.inputTokens.map((tok, idx) => (
              <span key={idx} className="w-7 h-7 rounded flex items-center justify-center bg-zinc-800 border border-zinc-700 text-xs font-bold text-zinc-300 font-mono">
                {tok}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex items-center gap-4 flex-1 w-full lg:w-auto">
          {blocks.map((block) => {
            const isLayer = block.id.startsWith('layer');
            const layerIndex = isLayer ? (block as any).layerIndex : 0;
            ...
            return (
              <div
                ...
                onClick={() => {
                  ...
                  if (isLayer) onSelectLayer(layerIndex);
                }}
                    ...
                    {Array.from({ length: numHeads }).map((_, h) => (
                      <span
                        ...
                        className={`text-[8px] px-1.5 py-0.5 rounded font-mono ${
                          selectedLayer === layerIndex && selectedHead === h
        ...
        <div className="flex flex-col items-center gap-1 min-w-[100px] text-center">
          ...
          <div className="flex gap-1.5 mt-1 bg-emerald-950/10 p-2 rounded-lg border border-emerald-900/30">
            {trace.predictions.map((tok, idx) => (
              <span key={idx} className="w-7 h-7 rounded flex items-center justify-center bg-emerald-900/20 border border-emerald-500/30 text-xs font-bold text-emerald-400 font-mono">
                {tok}
              </span>
            ))}
          </div>
