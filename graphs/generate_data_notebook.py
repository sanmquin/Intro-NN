...
def build_data_notebook():
    ...
    # Title & Introduction
    title_md = """# 0. Complex Graph Random Walk Traversal Dataset Generation and Topology Analysis
## Empirical Characterization of Random Walk Traversal Traces and Underlying Graph Topologies

### Executive Summary & Educational Motivation
In neural algorithmic reasoning, Depth-First Search (DFS) traces can sometimes be easily solved or exploited by sequence models due to strict recursive backtracking dynamics. Replacing DFS with **Random Walk** traversals forces models to navigate stochastic node transitions, requiring robust structural graph reasoning to extract true shortest paths.
...
1. **Easy Flavor (`graph_rw_easy_dataset.pt`)**: Sparse, tree-like topologies with low vertex connectivity and minimal cycle complexity.
...
Given graph $G = (V, E)$, starting at root node $s \\in V$, an agent performs a goal-terminated stochastic walk until reaching destination node $g \\in V$. Transitions $(t_k, t_{k+1})$ represent adjacent edge traversals $e \\in E$.
"""
...
"""
    ...
    cell2_md = """### Dataset Construction: Random Walk Traversal Sampling & Filtering
1. **Easy Graph Generator**: Sparse tree-like graph with 45-50 nodes and 1-3 extra edges.
2. **Hard Graph Generator**: Dense 2D grid/lattice with 50 nodes and extensive diagonal/cross edges creating multiple cycles.
...
def generate_rw_dataset(flavor='easy', target_samples=1000):
    dataset = []
    attempts = 0
...
print("Generating Easy Random Walk dataset (1000 samples)...")
t0 = time.time()
raw_easy = generate_rw_dataset('easy', 1000)
print(f"Generated {len(raw_easy)} Easy samples in {time.time() - t0:.2f}s.")

print("Generating Hard Random Walk dataset (1000 samples)...")
t0 = time.time()
raw_hard = generate_rw_dataset('hard', 1000)
print(f"Generated {len(raw_hard)} Hard samples in {time.time() - t0:.2f}s.")
"""
    cells.append(nbf.v4.new_code_cell(cell2_code))

    # Cell 3: Save Dataset to Drive / Local Directory
    cell3_md = """### Dataset Serialization to File
We serialize both dataset payloads using PyTorch `torch.save()`.
"""
    cells.append(nbf.v4.new_markdown_cell(cell3_md))

    cell3_code = """# Cell 3: Save Processed Datasets to Drive / Local Directory

def build_split_payload(raw_data):
    return {
        'train': raw_data[:750],
        'val': raw_data[750:875],
        'test': raw_data[875:1000],
...
print(f"Easy Random Walk Dataset saved to '{DATASET_PATH_EASY}' ({size_easy:.2f} MB).")
print(f"Hard Random Walk Dataset saved to '{DATASET_PATH_HARD}' ({size_hard:.2f} MB).")
"""
...
Computes topological and sequence bounds statistics across Easy and Hard dataset flavors.
"""
...
"""
    ...
    cell5_md = """### Analytical Figures: Distribution & Topologies
Visualizing input sequence length distributions and graph topology structures.
"""
...
"""
    ...
    # Cell 6: Summary
    cell6_md = """### Summary & Conclusions
1. **Random Walk Traversals**: Stochastic goal-terminated random walk traces of length $100 \\le K \\le 200$ and target shortest paths $20 \\le M \\le 50$.
2. **Two Flavors**: Easy (tree-like sparse graphs) and Hard (dense cycles and high connectivity).
